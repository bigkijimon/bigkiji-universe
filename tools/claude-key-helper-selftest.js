...
ok('an explicit key wins and the keychain is never opened', () => {
  ...
  assert.equal(readToken({ env: { ANTHROPIC_API_KEY: '   ' }, run: runOk }).source, 'keychain');
});
...
ok('the keychain is addressed by absolute path, never through $HOME', () => {
  ...
  assert.equal(loginKeychain('/Users/example'), '/Users/example/Library/Keychains/login.keychain-db');
  let args = null;
  readToken({ env: {}, run: (bin, argv) => { args = { bin, argv }; return ITEM; } });
...
ok('only the one approved field leaves the keychain item', () => {
  const result = readToken({ env: {}, run: runOk });
...
ok('an expired token fails loudly instead of printing something dead', () => {
  ...
  const result = readToken({ env: {}, run: () => expired, now: 2000000 });
  ...
  assert.match(readToken({ env: {}, run: () => soon, now: 1000000 }).error, /expired/);
  const fine = JSON.stringify({ claudeAiOauth: { accessToken: 'good', expiresAt: 1000000 + 600000 } });
  assert.equal(readToken({ env: {}, run: () => fine, now: 1000000 }).token, 'good');
});
...
ok('every failure says which one it was', () => {
  ...
  for (const [run, pattern] of cases) {
    const result = readToken({ env: {}, run });
