...
//
// 「作り直す」系がまるごと抜けていた。2026-08-14 の実測:
//
//     ダッシュボードを修正します     → TASK   （修正 が入っている）
//     ダッシュボードを再実装します   → TASK   （実装 が部分一致する）
//     ダッシュボードを再構築します   → CHAT   ← これ
//     ダッシュボードを作り直します   → CHAT
//
// オーナーの「H&Sの…ダッシュボードを再構築します。」が依頼にならず、lastRequest が
// 立たないので「はい」が指す先も無く、同じ確認質問を延々と繰り返した。
//
// 宣言形（〜します）を拾うのは新しい方針ではない。既存メンバーはすでにそうなっている。
// 抜けていた語を同じ列に足すだけで、判定の性格は変えていない。
// 誤爆の性質も既存と同じ（「修正のアイデアを考えたい」は前から TASK）で、肯定応答から
// 作られる run は gate:'plan' なので、誤読の代償は承認プロンプト1回に留まる。
const STRONG_ACTION = /(?:実装|修正|変更|追加|削除|再構築|作り直|刷新|再設計|build|implement|fix|refactor|commit|create|作って|直して|してください|してほしい)/i;
...
// STRONG_ACTION に足した「作り直す」系はここにも要る。isInspection は**最後に来た動詞**で
// 読み書きを決めるので、write 側の語彙に無いと位置が -1 になり、
// 「ログを見て再構築して」が read 最後＝閲覧と読まれる。書き換えの依頼が閲覧になる。
const WRITE_VERB = /(?:実装|修正|変更|追加|削除|再構築|作り直|刷新|再設計|消して|作って|作成|生成|書いて|書き換え|置き換え|更新|同期|移動|コピー|リネーム|整理|セットアップ|設定|導入|インストール|デプロイ|公開|再起動|起動|停止|バックアップ|直して|なおして|リファクタ)/;
