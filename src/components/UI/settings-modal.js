...
(() => {
  ...
  function renderSecurity(security = {}) {
    ...
    const recent = security.recent || []; root.querySelector('#securityRecent').innerHTML = recent.length ? recent.map((item) =>
      `<div class="security-row"><time>${esc(new Date(item.at).toLocaleTimeString())}</time><b>${esc(item.decision)}</b><span>${esc(item.provider || 'PiAgent')}</span><em>${esc(item.reason || 'Disclosure sealed')}</em></div>`).join('') : '<p class="settings-copy">No blocked actions in this daemon session.</p>';
  }
